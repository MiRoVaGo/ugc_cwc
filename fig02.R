...
global_e_cli <- data.frame(name = c("ERA5-Land", "mHM", "TerraClimate",
                                    "NCEP/NCAR R1"),
                           cli_mean = c(mean_era5, mean_e_obs,
                                        mean_terraclimate, mean_ncep_ncar),
                           cli_sd = c(sd_era5, sd_e_obs, sd_terraclimate,
                                      sd_ncep_ncar))
...
grdc$name <- "GRDC"
grdc <- grdc[, Year := year(date)][, P := sum(value), by = Year][, .(Year, P, name)] %>% unique()
...
mean_grdc <- mean(grdc[Year > 1980 & Year < 2011, P])
mean_e_obs <- mean(e_obs[Year > 1980 & Year < 2011, P])
mean_era5 <- mean(era5[Year > 1980 & Year < 2011, P])
mean_ncep_ncar <- mean(ncep_ncar[Year > 1980 & Year < 2011, P])
mean_terraclimate <- mean(terraclimate[Year > 1980 & Year < 2011, P])
sd_grdc <- sd(grdc[Year > 1980 & Year < 2011, P])
sd_e_obs <- sd(e_obs[Year > 1980 & Year < 2011, P])
sd_era5 <- sd(era5[Year > 1980 & Year < 2011, P])
sd_ncep_ncar <- sd(ncep_ncar[Year > 1980 & Year < 2011, P])
sd_terraclimate <- sd(terraclimate[Year > 1980 & Year < 2011, P])

global_q_cli <- data.frame(name = c("GRDC", "mHM", "TerraClimate", "ERA5-Land",
                                    "NCEP/NCAR R1"),
                           cli_mean = c(mean_grdc, mean_e_obs, mean_terraclimate,
                                        mean_era5, mean_ncep_ncar),
                           cli_sd = c(sd_grdc, sd_e_obs, sd_terraclimate, sd_era5,
                                      sd_ncep_ncar))
...
global_cor <- rbind(e_obs[, sim_qs := name][, .(Year, P, sim_qs)],
  ...
  merge(grdc[, obs := name][, .(Year, P, obs)], by = "Year",
        allow.cartesian = TRUE)
...
p10 <- ggplot(data = sim_q, aes(x = Year, y = P)) +
  ...
  geom_line(aes(color = "Data Sets")) +
  geom_line(color = "white") +
  geom_line(data = grdc, aes(y = 100*(P - mean_grdc)/mean_grdc,
                             color = "GRDC"), linewidth = 1) +
  scale_x_continuous(limits = c(1961, 2020), expand = c(0, 0), 
                     breaks = seq(1920, 2040, 10)) +
  scale_color_manual(name = NULL, values = c("GRDC" = "#7570b3",
                                             "Data Sets" = "gray")) +
  scale_y_continuous(limits = c(-150, 92), expand = c(0, 0), 
                     breaks = seq(-100, 100, 25)) +
  geom_hline(yintercept = 0) +
