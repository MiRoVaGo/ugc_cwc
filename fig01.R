...
ggplot(base) +
  ...
  geom_sf(data = elbe, fill = alpha("gray",0.4), color = 'gray', linewidth = 1) +
  geom_sf(data = danube, fill = alpha("black",0.4), color = NA) +
...
ggsave('fig01.pdf', width = 8.15, height = 5.01, dpi = 600, device = cairo_pdf)
